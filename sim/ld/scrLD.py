...
def training(p, nS):
    ...
    global X, y, beta0, plot_list, n, d, n_it_max, lr, n_runs, epsilon

    logger = logging.getLogger(__name__)
    logger.info('Beginning LD training ----- p = %d --- nS = %d' % (p,nS))
    start = time.time()
    np.random.seed(nS)
    res = trainLD(p, nS, lr=lr, X=X, y=y, beta0=beta0, plot_list=plot_list, n=n, d=d,
                   n_it_max=n_it_max, n_runs=n_runs, epsilon=epsilon)
    end = time.time()
    logger.info('End ----- p = %d --- nS = %d | Time elapsed LD: %f' % (p, nS, end-start))
    return res
...
if __name__ == '__main__':
    ...
    nS_ = np.arange(0,nS)
    #### Folder results
    subfolder = 'lr%.0e_eps%.0e_d%d_n%d_snr%.0e_nS%d_nrun%d_Nmax%.0e_np%d' % (lr, epsilon, d, n, snr, nS, n_runs, n_it_max, np_points)
